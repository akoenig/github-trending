 ...
 *
 */
Trending.prototype.$scrapeRepositories = function $scrapeRepositories (html) {
    var repositories = [];
 ...
 * 
 */
Trending.prototype.fetchRepositories = function fetchRepositories (language, callback) {
    var self = this;
    ...
    function onRequest (err, html) {
        ...
        repositories = self.$scrapeRepositories(html);
 ...
 * 
 */
module.exports = function Wrapper (language, callback) {
    ...
    trending.fetchRepositories(language, onFetch);
};

/**
 * Grabs all available languages for which trending repository data is available.
 *
 * Usage example:
 *
 * var trending = require('github-trending');
 ...
 *
 */
module.exports.languages = function languages (callback) {
    var trending = new Trending();
   
    function onFetch (err, languages) {
        if (err) {
            return callback(new VError(err, 'failed to get all available languages from GitHub.'));
        }
    ...
    trending.fetchLanguages(onFetch);
};
